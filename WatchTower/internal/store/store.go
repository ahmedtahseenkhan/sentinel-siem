...
// ── Native ticketing: grouping, SLA + audit trail ───────────────────────────

// FindOpenCaseByGroup returns the most recent open/investigating case for a
// group key, or nil — so repeat alerts append to it instead of spawning a new
// case.
...
// AppendAlertToCase adds an alert id to a case's alert_ids (idempotently).
...
// DeleteEntities removes all RBA + UEBA state for the given entity IDs across
// every per-entity table (risk events, notables, rolling scores, baselines,
// anomalies). Used to purge "ghost" entities — agent IDs from old/re-installed
// nodes that no longer appear in /api/agents and therefore can never resolve to
// a hostname on the dashboard. Returns the number of distinct entities that had
// a rolling risk score removed. The whole purge runs in one transaction.
func (s *Store) DeleteEntities(ids []string) (int, error) {
	if len(ids) == 0 {
		return 0, nil
	}
	...
	// Count distinct entities we're actually removing (union of the two score
	// tables) so the caller can report "removed N entities".
	var n int
	if err := tx.QueryRow(ctx, `
		SELECT COUNT(*) FROM (
			SELECT entity_id FROM rba_entity_risk  WHERE entity_id = ANY($1)
			UNION
			SELECT entity_id FROM ueba_risk_scores WHERE entity_id = ANY($1)
		) s`, ids).Scan(&n); err != nil {
		return 0, err
	}

	for _, q := range []string{
		`DELETE FROM rba_risk_events  WHERE entity_id = ANY($1)`,
		`DELETE FROM rba_notables     WHERE entity_id = ANY($1)`,
		`DELETE FROM rba_entity_risk  WHERE entity_id = ANY($1)`,
		`DELETE FROM ueba_baselines   WHERE entity_id = ANY($1)`,
		`DELETE FROM ueba_anomalies   WHERE entity_id = ANY($1)`,
		`DELETE FROM ueba_risk_scores WHERE entity_id = ANY($1)`,
	} {
